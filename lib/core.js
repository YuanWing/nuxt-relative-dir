...
const fs = require('fs');
const debug = require('debug')('nuxt-rd');
...
function readFile(filePath, targetDir) {
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    if (err) throw err;
    let targetFileContent = data.replace(/\/_nuxt\/(img\/)?/gim, `./${resourceDir}/`).replace(/\/(favicon.ico)/gim, './$1');
    fs.writeFile(targetFile, targetFileContent, 'utf8', (err) => {
      if (err) throw err;
      debug(`rewrite: ${targetFile}`);
    });
  });
}
